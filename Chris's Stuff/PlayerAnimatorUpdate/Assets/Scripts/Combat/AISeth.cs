...
//====================MOVING PHASE=====================
	...
	void OnCollisionStay2D(Collision2D playerC)
	{
		...
		{ 
			...
			{
				_player.GetComponent<PlayerReceivesDamage> ().InitiateCBT ((chargeDamage - armor).ToString ()).GetComponent<Animator> ().SetTrigger ("Hit"); //changed playerReceivesDamge
				_player.GetComponent<CombatScript> ().health -= (chargeDamage - armor);
			}
			else
			{ 
				armor = 0;
				_player.GetComponent<PlayerReceivesDamage> ().InitiateCBT (armor.ToString ()).GetComponent<Animator> ().SetTrigger ("Hit"); //changed playerReceivesDamge
			}
		}
	}
